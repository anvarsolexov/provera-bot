...
# 🗄 MA'LUMOTLAR BAZASINI SOZLASH (RENDERDA XATO BERMAYDIGAN REJIM)
# Renderda vaqtincha papka sifatida /tmp ishlatish eng xavfsiz yo'ldir
...
def add_order(user_id, name, service, phone):
    try:
        ...
        return int(time.time())  # Agar baza ishlamasa ham bot to'xtab qolmaydi, vaqtni ID qilib beradi
...
def send_welcome(message):
    try:
        ...
        if check_sub(user_id):
            bot.send_message(message.chat.id, "Assalomu aleykum! ProVera botiga xush kel
